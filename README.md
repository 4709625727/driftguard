...
This project was architected using a modular approach where separate components (data, model, drift, serving) were developed against a strict contract (docs/CONTRACT.md). This ensured that MLflow interactions remained isolated and model aliases were used correctly over deprecated stages. During integration, a methodology flaw was identified and corrected regarding the evaluation of retrained models, leading to the current honest evaluation framework. Details are available in docs/REVIEW.md.

## What I'd build next

- **Terraform** to stand up the MLflow server + API on ECS/Fargate.
- **Scheduled monitoring** (cron/Airflow) that runs the drift check on a rolling window and opens a PR or pages on drift.
...
Machine Learning Engineer with 4+ years of experience building, deploying, and monitoring production ML systems. Specialized in MLOps, deep learning, and Generative AI, with a focus on creating robust pipelines using Python, MLflow, and Docker.
